...
Revision 2. Dated 2026-08-19. Owner: Justin Goff. Steward: MetricMine Architect.
Status: FINAL system of record (Markdown master). Supersedes Rev 1 (2026-08-18) and the Rev 0.1 draft. Remaining work is asset staging only (B-06), which executes in the website repo.
...
| Artifact | File | Role |
|---|---|---|
| Markdown master | claude_MetricMine_Brand_Standards_08192026_Rev2.md | Normative specification (this file) |
...
Deprecated: the 08182026 experimental PNG-only board (superseded by the Board Master), the Rev 0.1 and Rev 1 markdown cuts, the Rev 1 Office records, and the crop-derived symbol-dark PNG.
...
- Fonts: Manrope (headings), Inter (body), JetBrains Mono (code). Theme tokens are in spec Section 3.7; use the CSS custom properties as written.
- Copy rules: short declarative sentences, no em dashes, no throughput or SLA claims. Thesis line: "Agents propose. Humans approve."
- Logos: use files under brand/ only. Never recolor, rebuild, or normalize logo art.
- Published diagrams follow spec Section 6.4: SVG source, paired light and dark, embedded subset fonts.
```
...
| B-06 | Asset staging: canonical renames, dedupe, brand directory per 8.2 | In progress, executes as a website repo PR |
